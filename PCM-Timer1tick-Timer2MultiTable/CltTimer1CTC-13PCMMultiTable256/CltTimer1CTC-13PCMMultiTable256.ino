...
/******** 正弦波、三角波、鋸齒波、方波二維數據陣列表 ********/
#define maxWaveform 4
#define maxSamplesNum 256
const unsigned char waveformsPGM[maxWaveform][maxSamplesNum]  PROGMEM = {
  // 正弦波
  {
...
static byte waveNow=2;
void tickMultiTableISR(void) {
  static byte index=0; //
  Timer2PWM.putPgm2Sound(speakerPin, waveformsPGM[waveNow]+index);
  index++;  
  // 微調
} 
...
void setup() {
  ...
  Timer1CTC.initTOP(16);
  Timer1CTC.attachInterrupt(tickMultiTableISR);
  interrupts();
}

void loop() { 
  delay(2000); 
  if (waveNow >= maxWaveform) {
    waveNow = 0;
  }
  waveNow++;
}
